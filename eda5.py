...
os.system("rm -r "+open_lane_path+"designs/adder/runs/"+this_tag)
acc_count = 0
    ...
    with open (open_lane_path+'designs/adder/runs/'+this_tag+'/reports/signoff/24-rcx_sta.area.rpt') as area_file:
        for data in area_file:
            if(data.find('Design area')!=-1):
                ...
                area = area.strip()
    with open (open_lane_path+'designs/adder/runs/'+this_tag+'/reports/signoff/24-rcx_sta.power.rpt') as pow_file:
        for data in pow_file:
            if(data.find('Total')!=-1):
                power = data.split()[-2]
    with open (open_lane_path+'designs/adder/runs/'+this_tag+'/reports/signoff/24-rcx_sta.max.rpt') as delay_file:
